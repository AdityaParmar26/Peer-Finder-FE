...
const NonTechnicalInterest = () => {
    ...
    if(loader && user.length === lenCheck){
        ...
        const rendertheresponse = user.map((item) => (
          <Card user={item} key={item._id} interest="non_technical_interest" isInterest = {true} id={item._id} obj = {handleCallback}/>
        ));
...
export default NonTechnicalInterest;