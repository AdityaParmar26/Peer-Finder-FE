...
const TechnicalInterest = () => {
    ...
    const callTechnicalPage = async ()=>{
        try {
          const res = await fetch("https://api-peer-finder.herokuapp.com/search/technical", {
            method: "GET",
            headers: {
      ...
      useEffect(() => {
        if(loader === false && lenCheck === 0) callTechnicalPage();
      }); 

    if(loader && user.length === lenCheck){
    
        const rendertheresponse = user.map((item) => (
          <Card user={item} key={item._id} interest="technical_interest" isInterest = {true} id={item._id} obj = {handleCallback}/>
        ));
